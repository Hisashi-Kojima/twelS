...
class Expression:
    ...
    def _remove_unnecessary_tags(soup: BeautifulSoup) -> str:
        ...
        Notes:
            ...
            2.  BeautifulSoupが<mspace/>を<mspace></mspace>に変換する。
            3.  この関数ではなくExpression._clean()のBeautifulSoup呼び出し前に
                <mspace/>を削除する方法も考えられるが、SnippetでもBeautifulSoupを呼び出しているので、
                Expressionに渡されるMathML中に<mspace></mspace>が含まれることがある。
            4.  （要確認）mspaceは数式の区切り文字として使用されることもあるようなので、
